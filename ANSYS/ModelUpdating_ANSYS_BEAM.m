...
% Slightly better results if 2nd actuator is placed at 248
act_loc_x = [50  248  450]/1000;
...
%%
omega_end= 1.2488e+03;
[~,idx] = min(abs(ffrf-omega_end));
% idx = length(G_exp)


CSAC_init = CSAC(G_exp(:,:,[1:126 440:idx]), G0(:,:,[1:126 440:idx]))
CSAC_updated = CSAC(G_exp(:,:,[1:idx]), G(:,:,[1:idx]))
CSAC_updated = CSAC(G_exp(:,:,[1:126 440:idx]), G(:,:,[1:126 440:idx]))
FRAC_init = FRAC(G_exp(:,:,[1:126 440:idx]), G0(:,:,[1:126 440:idx]))
FRAC_updated = FRAC(G_exp(:,:,[1:126 440:idx]), G(:,:,[1:126 440:idx]))
