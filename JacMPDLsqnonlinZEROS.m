...
for zero_i = 1:length(expModes.lambdaExpZeros)
    ...
    omegaExp = sqrt( expModes.lambdaExpZeros{zero_i} );

    n_modes = length(expModes.modeIndexZeros{zero_i});

    for i = 1 : n_modes
        if (normOpt == 1)
            simModes.psi_z{zero_i}(:,i) = simModes.psi_z{zero_i}(:,i) / simModes.psi_z{zero_i}(expModes.q(i), i);

        elseif (normOpt == 2)
            simModes.psi_z{zero_i}(:,i) = simModes.psi_z{zero_i}(:,i) / norm( simModes.psi_z{zero_i}(:, i) );
    ...
    for i = 1:length(expModes.modeIndexZeros{zero_i})
        for j = 1 : n_alpha
            ...
            Kzero(:,expModes.zerosDofs{zero_i}(2)) = [];
            dLambda(i,j) = simModes.psi_z{zero_i}(:,i)' * Kzero *...
                simModes.psi_z{zero_i}(:,i) / modalMass_z(i);
            if eigFreqOpt == 0
                d_ri_eigFreqTerm(i,j) = - dLambda(i,j) * expModes.lambdaWeightsZeros{zero_i}(i) ...
