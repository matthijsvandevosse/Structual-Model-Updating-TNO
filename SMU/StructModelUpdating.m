...
if(nargin < 4)
    ...
    if (length(optimzOpts.x0) ~= n_alpha+n_beta && updatingOpts.formID < 3)
...
if updatingOpts.formID == 1.9
    ...
    if ~isfield(expModes,'zerosDofs')
        expModes.zerosDofs = [];
    end

    for i = 1:length(expModes.zerosDofs)
        for j = 1:length(expModes.zerosDofs{i})
                expModes.zerosDofs{i}(j) =  find(orderIndex == expModes.zerosDofs{i}(j));
        end
    end
end
...
if( (updatingOpts.formID >= 2 && updatingOpts.formID < 3) || updatingOpts.formID >= 5)
    if(size(expModes.psiWeights,1) <  expModes.n_meas)
        ...
        for i = 1:expModes.n_meas
            expModes.psiWeights(i,:) =  psiWeights;
        end
    end
