...
if ( updatingOpts.formID ~= 3 )
    ...
    % Beam
    % [psi, lambda] = eigs(structModel.K,  structModel.M, max(matchedModeIndex), (2*2*pi)^2 , 'IsSymmetricDefinite', 1, 'Tolerance', 1e-3);

    % IRTF
    x'
    [psi, lambda] = eigs(structModel.K,  structModel.M, max(matchedModeIndex),  (structModel.FirstMode*2*pi)^2, 'IsSymmetricDefinite', 1, 'Tolerance', 1e-6);
    ...
    sim_mode_hz = round(sqrt(simModes.lambda')/2/pi)
    exp_mode_hz = round(sqrt(expModes.lambdaExp')/2/pi)
    %%
    if( updatingOpts.formID < 2.3 || updatingOpts.formID == 3.0) %&& ~(updatingOpts.formID == 1.4)
        ...
        for i = 1 : expModes.n_modes
            % We will normalize to the middle actuators [1 ... 18], since
            % the outer actuators are noise and mess up normalization

            % find max value of middle actuators
            [~, expModes.qm(i)] = max( abs( simModes.psi_m(1:18,i) ) );
    ...
    % Sign matching of the simulated and measured mode shapes (Extra check: This should already happen when the mode shape is normalized)
    for i = 1:(expModes.n_modes)
        dot_p = sign(dot(simModes.psi_m(:,i), expModes.psiExp(:,i)));
        if dot_p == -1
            simModes.psi_m(:,i) = -simModes.psi_m(:,i);
            simModes.psi(:,i) = -simModes.psi(:,i);
        end
    end
    ...
    if isfield(expModes,'Actuator_pos')
        ...
        figure(6)
        displaymode = 3;
        x_plot = expModes.Actuator_pos(:,1);
        y_plot = expModes.Actuator_pos(:,2);
        T = delaunay(x_plot,y_plot);
        T = unique(T,'rows');
        trisurf(T, x_plot, y_plot, simModes.psi_m(:, displaymode))
        hold on
        xlabel("X")
        ylabel("Y")
        title(['Simulated mode at ' num2str(sqrt(simModes.lambda(displaymode))/2/pi) 'hz' ])
        figure(5)
        trisurf(T, x_plot, y_plot, expModes.psiExp(:, displaymode))
        hold on
        xlabel("X")
        ylabel("Y")
