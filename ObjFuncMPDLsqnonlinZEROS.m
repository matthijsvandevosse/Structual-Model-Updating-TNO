function [r] =  ObjFuncMPDLsqnonlinZEROS(expModes, simModes, eigFreqOpt, normOpt, objOpt)
...
if (objOpt == 1)
    r_p = zeros(2 * n_modes, 1);
else
    if (normOpt == 1)
        r_p = zeros(n_meas * n_modes, 1);
    else
        r_p = zeros((n_meas + 1) * n_modes, 1);
    end
end
...
for i = 1 : n_modes
    ...
    if (objOpt == 1) % MAC value formulation
        ...
        r_p((i - 1) * 2 + 1 : i * 2) = [eigFreqTerm; MACterm];
        ...
        if (normOpt == 1) % Maximum entry is normalized to 1
            ...
            r_p((i - 1) * n_meas + 1 : i * n_meas) = [eigFreqTerm; psiDiff];
            ...
            r_p((i - 1) * (n_meas + 1) + 1 : i * (n_meas + 1)) = [eigFreqTerm; psiDiff];
        end
    end
end
    ...
    for zero_i = 1:length(expModes.zerosDofs)
        ...
        n_modes = length(expModes.modeIndexZeros{zero_i});


        for i = 1 : n_modes
            if (eigFreqOpt == 0)
                eigFreqTerm = expModes.lambdaWeightsZeros{zero_i}(i) * ...
                    (expModes.lambdaExpZeros{zero_i} - simModes.lambdaZeros{zero_i}) / expModes.lambdaExpZeros{zero_i};
            elseif (eigFreqOpt == 1)
                eigFreqTerm = expModes.lambdaWeightsZeros{zero_i}(i) * (omegaExp(i) - omegaSim(i)) / omegaExp(i);
            elseif (eigFreqOpt == 2)
